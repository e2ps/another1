<?php include '../nav/admin_nav.php';?>
				...
				<div class="row">
								...
								<tbody>
									<?php $n = 0; foreach ($notSeenMsg as $mssg): $n++ ?>
										<tr class="<?= (isset($_GET['msg_id']) && $_GET['msg_id'] == $mssg['inq_id'])? 'table-success':'' ?>"
											style="font-weight:<?= $mssg['inq_isSeen'] == 0? 'bold':''?>;"
											>
											...
											<td align="center">
												<a class="btn btn-info btn-sm" 
												   href="inquiry.php?page=inquiry&msg_id=<?= $mssg['inq_id'] ?>"
												   title="VIEW" 
												   >
													<span data-feather="eye" title="VIEW"></span>
												</a>
												<?php if ($mssg['inq_isSeen'] == 1): ?>
													<a class="btn btn-warning btn-sm" 
													   href="inquiry.php?page=inquiry&delete_id=<?= $mssg['inq_id'] ?>"
													   title="DELETE">
									...
									<?php foreach ($seenMsg as $mssg): $n++ ?>
										<tr class="<?= (isset($_GET['msg_id']) && $_GET['msg_id'] == $mssg['inq_id'])? 'table-success':'' ?>">
											...
											<td align="center">
												...
												<a class="btn btn-warning btn-sm" 
												   href="../page/admin.php?function=deletemsg&delete_id=<?= $mssg['inq_id'] ?>"
												   title="DELETE">
													<span data-feather="trash" title="DELETE"></span>
												</a>
											</td>
					...
					<?php if (isset($getMsg)): ?>
						<div class="col-md-8">
							<h2><small>From:</small> <?= empty($getMsg['inq_name'])? 'Anonymous':$getMsg['inq_name'] ?></h2>
							<div class="table-responsive">
								<hr>
								<p><?= $getMsg['inq_message'] ?></p>
								<hr/>
								<p>Email: <label> <?= $getMsg['inq_email'] ?></label><br/>
								<label>Contact Number: 0<?= $getMsg['inq_contact'] ?></label><br/>
								<label>Date: <?= date('M d, Y  h:i:s A', strtotime($getMsg['inq_date'])) ?></label></p>
							</div>
						</div>
					<?php elseif (isset($_GET['delete_id'])): ?>
						<div class="col-md-8">
							<h2><small>NOTE:</small> Message was deleted!</h2>
						</div>
					<?php endif ?>
				</div>
